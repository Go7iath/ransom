...
class PPAP:
    ...
    def play_bgm(self):
        ...
        if sys.platform.startswith("win"):
            # Windows: built-in MCI, loops natively
            try:
            ...
            # Kali / Linux: use whatever player is already present
            candidates = [
                ("ffplay", ["ffplay", "-nodisp", "-loglevel", "quiet",
                            "-loop", "0", BGM_PATH]),
                ("mpv",    ["mpv", "--no-video", "--loop=inf",
                            "--really-quiet", BGM_PATH]),
                ("mpg123", ["mpg123", "--loop", "-1", "-q", BGM_PATH]),
    ...
    def stop_bgm(self):
        # Windows: tell MCI to stop and unload the alias
        if self._bgm_alias:
            try:
                mci = ctypes.windll.winmm.mciSendStringW
        ...
        # Linux: terminate the child player
